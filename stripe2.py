...
def Paymnt(ccx):
	...
	if "20" in yy:
		yy = yy.split("20")[1]
	binmod = n[:6]
	with open("blockbin.txt", "r") as file:
		blocklist = file.read().splitlines()
	if binmod in blocklist:
		return 'BLACKLISTED_BIN: This BIN is blacklisted.'
	file_path = "links.txt"
	with open(file_path, 'r') as file:
        	    url_stripe = [line.strip() for line in file if line.strip()]
        	    if not url_stripe:
